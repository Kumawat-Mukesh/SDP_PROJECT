...
if ($_POST) {
    ...
    $query = mysqli_query($connection, "insert into tbl_donation(ngo_id,user_id ,item_requirement_id,donation_details,donation_status,volunteer_id) values('{$ngo_id}','{$user_id}' ,'{$item_requirement_id}','{$donation_details}','{$donation_status}','{$volunteer_id}')");
    if ($query) {
        echo "<script>alert('Donation added to the database');window.location='donation_form.php'</script>";
    }
}
?>
<!DOCTYPE html>
                    ...
                    <div class="tile-body">
                        <form method="post" class="row" id="donation_form_js">
                            <div class="mb-3 col-md-3">
                                ...
                                <textarea name="donation_details" class="form-control" cols="3" rows="5" placeholder="Enter donation details" required></textarea>
                                <br>
                                <label class="form-label">Donation Status</label>
                                <input class="form-control" type="text" name="donation_status" placeholder="Enter status" required>
                                <br>
