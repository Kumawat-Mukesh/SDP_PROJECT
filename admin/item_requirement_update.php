...
$item_requirement_select=mysqli_query($connection,"select *from tbl_item_requirement where item_requirement_id={$edit_id}");
$item_requirement_row=mysqli_fetch_array($item_requirement_select);
if ($_POST) {
    //$ngo_id = $_POST['ngo_id'];
    $item_requirement_details = $_POST['item_requirement_details'];
    $item_requirement_status = $_POST['item_requirement_status'];
    $query = mysqli_query($connection, "update tbl_item_requirement set item_requirement_details='{$item_requirement_details}',item_requirement_status='{$item_requirement_status}' where item_requirement_id='{$edit_id}'");
    if ($query) {
        echo "<script>alert('Item Requirement updated');window.location='item_requirement_information.php'</script>";
    }
}
?>
<!DOCTYPE html>
                    ...
                    <div class="tile-body">
                        <form method="post" class="row" id="item_requirement_form_js">
                            <div class="mb-3 col-md-3">
                                ...
                                <textarea name="item_requirement_details" class="form-control" cols="3" rows="5" placeholder="Enter requirement details" required><?php echo $item_requirement_row['item_requirement_details'];?></textarea>
                                <br>
                                <br>
                                <label class="form-label">Requirement Status</label>
                                <input class="form-control" type="text" name="item_requirement_status" placeholder="Enter status" value="<?php echo $item_requirement_row['item_requirement_status'];?>" required>
                                <br>
